...
export class Decorate extends Events {
  ...
  _retrieveMapEntries(entry, at, arg) {
    ...
    return this._retrieveMapKeys(entry, at, arg).pipe(switchMap(keys => combineLatest(arrayChunk(keys, PAGE_SIZE).map(keys => query(keys))).pipe(map(valsArr => arrayFlatten(valsArr).map((value, index) => [keys[index], value])))));
  }

  _retrieveMapEntriesPaged(entry, opts) {
    return this._retrieveMapKeysPaged(entry, opts).pipe(switchMap(keys => this._rpcCore.state.queryStorageAt(keys).pipe(map(valsArr => valsArr.map((value, index) => [keys[index], value])))));
  }
