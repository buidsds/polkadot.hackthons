...
class Decorate extends _Events.Events {
  ...
  _retrieveMapEntries(entry, at, arg) {
    ...
    return this._retrieveMapKeys(entry, at, arg).pipe((0, _operators.switchMap)(keys => (0, _xRxjs.combineLatest)((0, _util2.arrayChunk)(keys, PAGE_SIZE).map(keys => query(keys))).pipe((0, _operators.map)(valsArr => (0, _util2.arrayFlatten)(valsArr).map((value, index) => [keys[index], value])))));
  }

  _retrieveMapEntriesPaged(entry, opts) {
    return this._retrieveMapKeysPaged(entry, opts).pipe((0, _operators.switchMap)(keys => this._rpcCore.state.queryStorageAt(keys).pipe((0, _operators.map)(valsArr => valsArr.map((value, index) => [keys[index], value])))));
  }
